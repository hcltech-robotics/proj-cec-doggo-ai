...
class GaugeReaderNode(GaugeReaderParent):
    ...
    def __init__(self):
        ...
        self._use_math_reading = (
            self.get_parameter(f'{self._namespace}.use_math').get_parameter_value().bool_value
        )
        self._image_topic = (
            self.get_parameter(f'{self._namespace}.image_topic').get_parameter_value().string_value
        )
        ...
        self._scaling_min = (
            self.get_parameter(f'{self._namespace}.scaling_min').get_parameter_value().double_value
        )
        self._scaling_max = (
            self.get_parameter(f'{self._namespace}.scaling_max').get_parameter_value().double_value
        )
    ...
    def detect(self, cv_image):
        ...
        r = requests.post(self.model_server_url + "/detect", json={'image': b64}, headers=headers)
    ...
    def read(self, crop, bbox):
        ...
        b64 = base64.b64encode(buf).decode('utf-8')
        headers = {'Authorization': f'Bearer {self.token}'}
        r = requests.post(self.model_server_url + "/read", json={'image': b64, 'bbox': bbox}, headers=headers)

        if r.status_code == 200:
            return r.json().get('reading', 0.0)
        else: 
            self.get_logger().error(f"Error while calling read: {r.status_code} - {r.text}")
            raise Exception(f"Error while calling detect: {r.status_code} - {r.text}")
