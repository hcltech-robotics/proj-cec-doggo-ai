...
class GaugeReader(Node):
    ...
    def synced_callback(self, image_msg, detections_msg):
        #self.get_logger().warning(f"Got msg {detections_msg}")
        """
        This callback is triggered only when both image_msg and detections_msg have the same header timestamp.
        It converts the image to a tensor, extracts the gauge (class_id == "2"), extracts a normalized bounding box for the 
        needle from detections (class_id == "1"), and then passes both to the model to compute the gauge reading.
        """
        ...
        for det in detections_msg.detections:
            for hyp in det.results:
                # Ensure the hypothesis is of type ObjectHypothesisWithPose (its class_id should be a string).
        ...
        if g_x_min < 0 or g_y_min < 0 or g_x_max > width or g_y_max > height:
            self.get_logger().warning("Gauge bounding box is out of image bounds, skipping observation.")
            return
        ...
        if n_x_min < g_x_min or n_y_min < g_y_min or n_x_max > g_x_max or n_y_max > g_y_max:
            self.get_logger().warning("Needle bounding box is not fully inside the gauge, skipping observation.")
            return
